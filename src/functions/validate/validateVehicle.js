...
export const validateVehicle = async ({ data, Model, msg }) => {
  ...
  if (data.place !== 'No asignado') {
    const isExistByPlace = await Model.findOne({
      where: { place: data.place },
    });
    val({ val: isExistByPlace, errors, msg, name: 'serial' });
  }
